...
def coerce_roadmap_payload(
    ...
    if not isinstance(levels, list):
        raise JSONParseError(
            "Roadmap must contain a levels list."
        )
...
def coerce_lesson_payload(
    ...
    if not isinstance(question, dict):
        raise JSONParseError(
            "Lesson question must be an object."
        )

    question_type = (
        question.get("type")
        or request["question_type"]
    )
...
def coerce_evaluation_payload(
    payload: dict[str, Any],
) -> dict[str, Any]:

    if "is_correct" not in payload:
        raise JSONParseError(
            "Evaluation payload is missing is_correct."
        )
...
def coerce_question_ranking_payload(
    payload: dict[str, Any],
) -> dict[str, Any]:
...
def coerce_hard_question_payload(
    payload: dict[str, Any],
) -> dict[str, Any]:
